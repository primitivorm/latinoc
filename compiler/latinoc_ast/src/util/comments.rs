...
use latinoc_span::{BytePos, CharPos, FileName, Pos, Symbol};
...
pub fn gather_comments(sm: &SourceMap, path: FileName, src: String) -> Vec<Comment> {
    ...
    for token in latinoc_lexer::tokenize(&text[pos..]) {
        let token_text = &text[pos..pos + token.len];
        match token.kind {
            latinoc_lexer::TokenKind::Whitespace => {
            ...
            latinoc_lexer::TokenKind::BlockComment { doc_style, .. } => {

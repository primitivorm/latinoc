...
use latinoc_ast::attr::MarkedAttrs;
use latinoc_ast::ptr::P;
use latinoc_ast::token::{self, Nonterminal};
use latinoc_ast::tokenstream::{CanSynthesizeMissingTokens, TokenStream};
use latinoc_ast::visit::{AssocCtxt, Visitor};
use latinoc_ast::{self as ast, AstLike, Attribute, Item, NodeId, PatKind};
use latinoc_parse::{self, nt_to_tokenstream, parser, MACRO_ARGUMENTS};
use latinoc_span::def_id::{CrateNum, DefId, LocalDefId};
use latinoc_span::edition::Edition;
use latinoc_span::hygiene::{AstPass, ExpnData, ExpnKind, LocalExpnId};
use latinoc_span::source_map::SourceMap;
use latinoc_span::symbol::{kw, sym, Ident, Symbol};
use latinoc_span::{FileName, MultiSpan, Span, DUMMY_SP};
...
crate use latinoc_span::hygiene::MacroKind;
...
impl<'a> ExtCtxt<'a> {
    ...
    pub fn new_parser_from_tts(&self, stream: TokenStream) -> parser::Parser<'a> {
        latinoc_parse::stream_to_parser(&self.sess.parse_sess, stream, MACRO_ARGUMENTS)
    }
    ...
    ///
    /// FIXME: move this to `latinoc_builtin_macros` and make it private.
    pub fn resolve_path(
        &self,

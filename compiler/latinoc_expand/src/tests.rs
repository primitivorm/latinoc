...
use latinoc_parse::{new_parser_from_source_str, parser::Parser, source_file_to_stream};
use rustc_session::parse::ParseSess;
use latinoc_span::create_default_session_if_not_set_then;
...
crate fn matches_codepattern(a: &str, b: &str) -> bool {
    ...
    loop {
        let (a, b) = match (a_iter.peek(), b_iter.peek()) {
            ...
            (None, _) => return false,
            (Some(&a), None) => {
                if latinoc_lexer::is_whitespace(a) {
        ...
        if latinoc_lexer::is_whitespace(a) && latinoc_lexer::is_whitespace(b) {
            // Skip whitespace for `a` and `b`.
            scan_for_non_ws_or_end(&mut a_iter);
            scan_for_non_ws_or_end(&mut b_iter);
        } else if latinoc_lexer::is_whitespace(a) {
    ...
    a_iter.all(latinoc_lexer::is_whitespace)
}

/// Advances the given peekable `Iterator` until it reaches a non-whitespace character.
fn scan_for_non_ws_or_end<I: Iterator<Item = char>>(iter: &mut Peekable<I>) {
    while iter.peek().copied().map(latinoc_lexer::is_whitespace) == Some(true) {
        iter.next();
    }
}

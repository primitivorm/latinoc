...
use latinoc_ast::token::{self, NonterminalKind, NtTT, Token, TokenKind::*};
use latinoc_ast::tokenstream::{DelimSpan, TokenStream};
use latinoc_ast::{NodeId, DUMMY_NODE_ID};
...
use latinoc_span::symbol::{kw, sym, Ident, MacroRulesNormalizedIdent};
...
pub fn compile_declarative_macro(
    ...
    let parser = Parser::new(&sess.parse_sess, body, true, latinoc_parse::MACRO_ARGUMENTS);
...
fn parser_from_cx(sess: &ParseSess, tts: TokenStream) -> Parser<'_> {
    Parser::new(sess, tts, true, latinoc_parse::MACRO_ARGUMENTS)
}

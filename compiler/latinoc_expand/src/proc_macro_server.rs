...
impl Ident {
    fn new(sess: &ParseSess, sym: Symbol, is_raw: bool, span: Span) -> Ident {
        ...
        if !latinoc_lexer::is_ident(&string) {
...
impl server::Literal for Rustc<'_, '_> {
    fn from_str(&mut self, s: &str) -> Result<Self::Literal, ()> {
        ...
        let mut parser = latinoc_parse::new_parser_from_source_str(self.sess(), name, s.to_owned());
...
fn ident_name_compatibility_hack(
    ...
    orig_span: Span,
    rustc: &mut Rustc<'_, '_>,
) -> Option<(latinoc_span::symbol::Ident, bool)> {

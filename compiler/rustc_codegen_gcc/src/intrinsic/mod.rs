...
use rustc_codegen_ssa::common::{span_invalid_monomorphization_error, IntPredicate};
use rustc_codegen_ssa::mir::operand::{OperandRef, OperandValue};
use rustc_codegen_ssa::mir::place::PlaceRef;
use rustc_codegen_ssa::traits::{
    ArgAbiMethods, BaseTypeMethods, BuilderMethods, ConstMethods, IntrinsicCallMethods,
};
                ...
                match int_type_width_signed(ty, self) {
                    Some((width, signed)) => match name {
                        sym::ctlz | sym::cttz => {
                            ...
                            let cond = self.cx.context.new_comparison(
                                None,
                                ComparisonOp::Equals,
                                arg,
                                zero,
                            ...
                            let zero_result =
                                self.cx.context.new_rvalue_from_long(arg.get_type(), width as i64);
                        ...
                        sym::bswap => {
                            if width == 8 {
                                ...
                                let bswap = self
                                    .cx
                                    .context
                                    .get_builtin_function(&format!("__builtin_bswap{}", width));
...
impl<'gcc, 'tcx> ArgAbiExt<'gcc, 'tcx> for ArgAbi<'tcx, Ty<'tcx>> {
    ...
    fn store_fn_arg<'a>(
        ...
        match self.mode {
            ...
            PassMode::Direct(_)
            | PassMode::Indirect { extra_attrs: None, .. }
            | PassMode::Cast(_) => {
                let next_arg = next();
...
impl<'a, 'gcc, 'tcx> Builder<'a, 'gcc, 'tcx> {
    fn bit_reverse(&mut self, width: u64, value: RValue<'gcc>) -> RValue<'gcc> {
        ...
        let result = match width {
            ...
            64 => {
                ...
                let right = self
                    .and(step1, context.new_rvalue_from_long(typ, 0xFFFE0000FFFE0000u64 as i64)); // TODO(antoyo): transmute the number instead?
    ...
    fn count_leading_zeroes(&self, width: u64, arg: RValue<'gcc>) -> RValue<'gcc> {
        ...
        let count_leading_zeroes = if arg_type.is_uint(&self.cx) {
            "__builtin_clz"
            ...
            let result =
                self.current_func().new_local(None, array_type, "count_loading_zeroes_results");
            ...
            let first_value =
                self.context.new_cast(None, self.context.new_call(None, clzll, &[high]), arg_type);
            self.llbb().add_assignment(None, first_elem, first_value);

            let second_elem = self.context.new_array_access(None, result, one);
            let second_value =
                self.context.new_cast(None, self.context.new_call(None, clzll, &[low]), arg_type)
                    + sixty_four;
            self.llbb().add_assignment(None, second_elem, second_value);
            ...
            self.llbb().add_assignment(None, third_elem, third_value);

            let not_high =
                self.context.new_unary_op(None, UnaryOp::LogicalNegate, self.u64_type, high);
            let not_low =
                self.context.new_unary_op(None, UnaryOp::LogicalNegate, self.u64_type, low);
    ...
    fn count_trailing_zeroes(&self, _width: u64, arg: RValue<'gcc>) -> RValue<'gcc> {
        ...
        let arg_type = arg.get_type();
        let (count_trailing_zeroes, expected_type) = if arg_type.is_uchar(&self.cx)
            ...
            || arg_type.is_uint(&self.cx)
        {
            ...
            let first_elem = self.context.new_array_access(None, result, zero);
            let first_value =
                self.context.new_cast(None, self.context.new_call(None, ctzll, &[low]), arg_type);
            self.llbb().add_assignment(None, first_elem, first_value);

            let second_elem = self.context.new_array_access(None, result, one);
            let second_value =
                self.context.new_cast(None, self.context.new_call(None, ctzll, &[high]), arg_type)
                    + sixty_four;
            self.llbb().add_assignment(None, second_elem, second_value);
    ...
    fn saturating_add(
        ...
        if signed {
            ...
            let shifted = self.context.new_cast(None, lhs, unsigned_type)
                >> self.context.new_rvalue_from_int(unsigned_type, width as i32 - 1);
            let uint_max = self.context.new_unary_op(
                None,
            ...
            then_block.add_assignment(
                None,
                res,
                self.context.new_cast(None, shifted + int_max, result_type),
            );
            then_block.end_with_jump(None, after_block);
            ...
            *self.current_block.borrow_mut() = Some(after_block);
            ...
            let value = self.context.new_unary_op(
                None,
                UnaryOp::Minus,
                res_type,
                self.context.new_cast(None, cond, res_type),
            );
            res | value
        }
    }
    ...
    fn saturating_sub(
        ...
        if signed {
            ...
            *self.current_block.borrow_mut() = Some(after_block);
            ...
            let res = lhs - rhs;
            let comparison =
                self.context.new_comparison(None, ComparisonOp::LessThanEquals, res, lhs);
            let comparison = self.context.new_cast(None, comparison, lhs.get_type());
            let unary_op =
                self.context.new_unary_op(None, UnaryOp::Minus, comparison.get_type(), comparison);
            self.and(res, unary_op)
        }
    }
}

fn try_intrinsic<'gcc, 'tcx>(
    bx: &mut Builder<'_, 'gcc, 'tcx>,
    try_func: RValue<'gcc>,
    data: RValue<'gcc>,

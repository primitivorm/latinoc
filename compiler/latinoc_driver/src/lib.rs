        ...
        for req in &sess.opts.prints {
            match *req {
                ...
                FileNames | CrateName => {
                    ...
                    let t_outputs = latinoc_interface::util::build_output_filenames(
...
fn parse_crate_attrs<'a>(sess: &'a Session, input: &Input) -> PResult<'a, Vec<ast::Attribute>> {
    match input {
        Input::File(ifile) => latinoc_parse::parse_crate_attrs_from_file(ifile, &sess.parse_sess),
        Input::Str { name, input } => latinoc_parse::parse_crate_attrs_from_source_str(
            name.clone(),
            input.clone(),
            &sess.parse_sess,
        ),

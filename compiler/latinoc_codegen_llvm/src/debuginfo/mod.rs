...
use latinoc_span::{self, BytePos, Pos, SourceFile, SourceFileAndLine, Span};
...
// `latinoc_span::Loc` rather than `DILocation`, perhaps by making
    ...
    fn dbg_scope_fn(
        ...
        fn get_function_signature<'ll, 'tcx>(
            ...
            signature.push(if fn_abi.ret.is_ignore() {
                ...
                Some(type_metadata(cx, fn_abi.ret.layout.ty, latinoc_span::DUMMY_SP))
            });
            ...
            if cx.sess().target.is_like_msvc {
                ...
                signature.extend(fn_abi.args.iter().map(|arg| {
                    ...
                    Some(type_metadata(cx, t, latinoc_span::DUMMY_SP))
                }));
            } else {
                signature.extend(
                    fn_abi
                        .args
                        .iter()
                        .map(|arg| Some(type_metadata(cx, arg.layout.ty, latinoc_span::DUMMY_SP))),
                );
            }
                ...
                if cx.tcx.trait_id_of_impl(impl_def_id).is_none() {
                    ...
                    match impl_self_ty.kind() {
                        ty::Adt(def, ..) if !def.is_box() => {
                            ...
                            {
                                Some(type_metadata(cx, impl_self_ty, latinoc_span::DUMMY_SP))
                            } else {

...
use latinoc_ast::util::{classify, parser};
use latinoc_ast::{ExprKind, StmtKind};
...
trait UnusedDelimLint {
    ...
    fn check_expr(&mut self, cx: &EarlyContext<'_>, e: &ast::Expr) {
        ...
        let (value, ctx, followed_by_block, left_pos, right_pos) = match e.kind {
            ...
            {
                let left = e.span.lo() + latinoc_span::BytePos(2);
            ...
            While(ref cond, ref block, ..)
                if !matches!(cond.kind, Let(_, _, _))
                    || Self::LINT_EXPR_IN_PATTERN_MATCHING_CTX =>
            {
                let left = e.span.lo() + latinoc_span::BytePos(5);
                let right = block.span.lo();
            ...
            Ret(Some(ref value)) => {
                let left = e.span.lo() + latinoc_span::BytePos(3);
                ...
                if !Self::is_expr_delims_necessary(inner, followed_by_block, followed_by_else)
                    ...
                    && (ctx != UnusedDelimsCtx::LetScrutineeExpr
                        || !matches!(inner.kind, ast::ExprKind::Binary(
                                latinoc_span::source_map::Spanned { node, .. },
                                _,
                                _,
                            ) if node.lazy()))
                {

...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for line in profile.lines() {
        ...
        // Trim start
        if let Some(index) = stack.find("latinoc_interface::passes::configure_and_expand") {
            stack = &stack[index..];
        } else if let Some(index) = stack.find("latinoc_interface::passes::analysis") {
            stack = &stack[index..];
        } else if let Some(index) = stack.find("latinoc_interface::passes::start_codegen") {
            stack = &stack[index..];
        } else if let Some(index) = stack.find("latinoc_interface::queries::Linker::link") {
            stack = &stack[index..];
        }

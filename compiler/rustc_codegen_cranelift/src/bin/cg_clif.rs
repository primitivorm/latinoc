...
static DEFAULT_HOOK: SyncLazy<Box<dyn Fn(&panic::PanicInfo<'_>) + Sync + Send + 'static>> =
    SyncLazy::new(|| {
        ...
        panic::set_hook(Box::new(|info| {
            ...
            latinoc_driver::report_ice(info, BUG_REPORT_URL);
        }));
        hook
    });
...
impl latinoc_driver::Callbacks for CraneliftPassesCallbacks {
...
fn main() {
    ...
    let exit_code = latinoc_driver::catch_with_exit_code(|| {
        let args = std::env::args_os()
            ...
            .collect::<Vec<_>>();
        let mut run_compiler = latinoc_driver::RunCompiler::new(&args, &mut callbacks);

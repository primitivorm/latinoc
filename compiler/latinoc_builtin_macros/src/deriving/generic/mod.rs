...
use latinoc_ast::{self as ast, BinOpKind, EnumDef, Expr, Generics, PatKind};
use latinoc_ast::{GenericArg, GenericParamKind, VariantData};
...
impl<'a> TraitDef<'a> {
    ...
    fn create_derived_impl(
        ...
        let unused_qual = {
            let word = latinoc_ast::attr::mk_nested_word_item(Ident::new(
                sym::unused_qualifications,
                self.span,
            ));
            let list = latinoc_ast::attr::mk_list_item(Ident::new(sym::allow, self.span), vec![word]);
            cx.attribute(list)
        };

        ...
        pub fn to_resolver_outputs(resolver: Rc<RefCell<BoxedResolver>>) -> ResolverOutputs {
            match Rc::try_unwrap(resolver) {
                ...
                Err(resolver) => resolver
                    .borrow_mut()
                    .access(|resolver| resolver.clone_outputs()),
            }
        }
    }
}
...
pub fn configure_and_expand(
    ...
    if !matches!(
        sess.opts.pretty,
        Some(PpMode::Source(PpSourceMode::EveryBodyLoops))
    ) {
...
pub fn lower_to_hir<'res, 'tcx>(
    ...
    let hir_crate = rustc_ast_lowering::lower_crate(
        ...
        &*krate,
        resolver,
        latinoc_parse::nt_to_tokenstream,
        arena,
    );
...
fn analysis(tcx: TyCtxt<'_>, (): ()) -> Result<()> {
    ...
    sess.time("MIR_borrow_checking", || {
        tcx.hir()
            .par_body_owners(|def_id| tcx.ensure().mir_borrowck(def_id));
    });
...
fn encode_and_write_metadata(
    ...
    if need_metadata_file {
        ...
        let metadata_tmpdir = TempFileBuilder::new()
            ...
            .unwrap_or_else(|err| {
                tcx.sess
                    .fatal(&format!("couldn't create a temp dir: {}", err))
            });

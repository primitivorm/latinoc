...
use latinoc_ast::tokenstream::{DelimSpan, TokenTree};
use latinoc_ast::{self as ast, Attribute, MacArgs, MacDelimiter, MetaItem, MetaItemKind};
...
fn emit_malformed_attribute(
    ...
    let should_warn = |name| {
        matches!(
            name,
            sym::doc | sym::ignore | sym::inline | sym::link | sym::test | sym::bench
        )
    };
    ...
    let mut first = true;
    let inner = if attr.style == ast::AttrStyle::Inner {
        "!"
    } else {
        ""
    };
...
pub fn emit_fatal_malformed_builtin_attribute(
    ...
    let template = BUILTIN_ATTRIBUTE_MAP
        .get(&name)
        .expect("builtin attr defined")
        .template;
    emit_malformed_attribute(sess, attr, name, template);
